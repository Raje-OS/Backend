...
public class LibraryCommandServiceImpl implements LibraryCommandService {
    ...
    public LibraryCommandServiceImpl(LibraryRepository libraryRepository, TokenService tokenService, HashingService hashingService) {
    ...
    public Library handle(SignInLibraryCommand command) {
        ...
        var token = tokenService.generateToken(library.getId());
        System.out.println("Token generado para librería: " + token);
        return library;
    }

    @Override
    public Object generateTokenForLibrary(Library library) {
